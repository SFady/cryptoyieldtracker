...
export async function POST(req) {
  const { amount, destination } = await req.json().catch(() => ({}));
  ...
  // Destination = adresse fournie, sinon l'adresse du wallet pool 2 lui-même
  const dest = destination ?? wallet.address;
  ...
  try {
    ...
    if (json.status === "ok")
      return Response.json({ ok: true, amount, destination: dest, from: "pool2-HL", result: json });
