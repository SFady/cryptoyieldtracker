...
function HlResizePositionItem({ isOpen, onToggle, borderBottom }) {
  ...
  async function submit() {
    ...
    setResult(null);
    try {
      const res  = await fetch("/api/hyperliquid-resize", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ marginUsd: parseFloat(marginUsd), leverage: 4 }),
      });
      const json = await res.json();
  ...
  const btnLabel  = isLoading ? "..." : confirming ? "⚠ OK ?" : "APPLIQUER";
      ...
      {isOpen && (
        <div style={{ padding: "0 14px 12px", display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ fontFamily: "monospace", fontSize: "0.6rem", color: "#44446a" }}>
            Nouvelle marge (USDC) · levier ×4 · calcule le delta et ajuste sans toucher les TP/SL
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <input
              ...
              placeholder="ex: 20"
              value={marginUsd}
              onChange={e => { setMarginUsd(e.target.value); setConfirming(false); setStatus(null); setResult(null); }}
              disabled={isLoading}
              style={{
                flex: 1,
                fontFamily: "monospace",
            ...
            <button
              ...
              disabled={isLoading || !marginUsd || parseFloat(marginUsd) <= 0}
              style={{
                padding: "7px 14px",
                background: confirming ? "rgba(240,180,41,0.15)" : `${color}15`,
                border: `1px solid ${btnColor}66`,
                borderRadius: 5,
                color: (isLoading || !marginUsd) ? `${btnColor}44` : btnColor,
                fontFamily: "monospace",
                fontSize: "0.75rem",
                fontWeight: 700,
                cursor: (isLoading || !marginUsd) ? "default" : "pointer",
                whiteSpace: "nowrap",
                transition: "all 0.15s",
              }}
            >
          ...
          {result && (
            <pre style={{
              ...
              color: status === "ok" ? "#00e5a0" : "#c97070",
              background: "rgba(0,0,0,0.25)",
              border: `1px solid ${status === "ok" ? "rgba(0,229,160,0.2)" : "rgba(201,112,112,0.2)"}`,
              borderRadius: 6,
              padding: "10px 12px",
              margin: 0,
              whiteSpace: "pre-wrap",
              ...
              {status === "ok"
                ? `✓ ${result.action === "increase" ? "▲ Augmenté" : result.action === "decrease" ? "▼ Réduit" : "Inchangé"}\n` +
                  `Avant : ${result.currentEth?.toFixed(4)} ETH\nAprès : ${result.targetEth?.toFixed(4)} ETH\nDelta : ${result.deltaEth > 0 ? "+" : ""}${result.deltaEth?.toFixed(4)} ETH\n@ $${result.ethPrice?.toFixed(1)}`
      ...
      {open && (
        <div style={{
          ...
          <Pool2TotalItem
            ...
            borderBottom
          />
          <HlResizePositionItem
            isOpen={expanded === "hl-resize"}
            onToggle={() => toggleItem("hl-resize")}
            borderBottom
          />
          <StartItem
